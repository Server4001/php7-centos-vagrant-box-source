class php_install {
  ...
  exec { 'install_php_ds':
    command => 'pecl install ds',
    path => '/usr/bin:/usr/local/bin:/usr/sbin:/usr/local/sbin:/bin',
    creates => '/usr/lib64/php/modules/ds.so',
    require => Class['php::cli'],
  }->

  file { 'php_ds_config':
    path => '/etc/php.d/j-20-ds.ini',
    content => template('php_install/ds.ini.erb'),
    owner => 'root',
    group => 'root',
    mode => 0644,
  ...
  exec { 'move_php_json_config':
    command => 'mv /etc/php.d/json.ini /etc/php.d/j-10-json.ini',
    path => '/usr/bin:/usr/local/bin:/usr/sbin:/usr/local/sbin:/bin',
    creates => '/etc/php.d/j-10-json.ini',
    notify => Service['php-fpm'],
  }
}
